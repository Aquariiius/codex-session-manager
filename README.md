...
Codex 会话管理是一款完全由VibeCoding开发的基于 Tauri + Rust 的本地桌面工具，用于核对 Codex 中的项目根目录、状态数据库工作目录和 JSONL 会话日志工作目录，并在项目迁移后安全修复不一致的路径。

应用直接读取本机 Codex 数据，不上传会话内容，也不依赖远程服务。除路径审计外，提供会话导入导出、会话与遗留日志删除、操作回退和备份历史管理。

## ✨ 功能特性

### 🔎 会话审计

- **三方路径核对**：比较项目根目录、`threads.cwd` 与 JSONL 中的 `session_meta.cwd`
- **项目归属保留**：读取 Codex 保存的项目归属关系，项目迁移后仍能识别原会话
- **状态筛选**：按路径一致、路径不一致、未归属项目和日志缺失快速筛选
- **遗留记录识别**：查找已不被状态数据库引用的 JSONL 会话日志

### 🛠️ 修复与回退

- **精确路径修复**：只更新数据库及 JSONL 中名为 `cwd` 的工作目录字段
- **子代理级联修复**：可选递归同步主会话派生的全部子代理
- **修复前备份**：修改前保存数据库、WAL/SHM 文件及受影响日志
- **修复历史回退**：从操作清单恢复数据库与会话日志，并在回退前再次备份当前状态

### 📦 会话迁移

- **按项目选择会话**：支持项目折叠、全部折叠/展开和批量勾选
- **完整导出包**：导出精简的数据库快照、项目配置、会话记录、子代理关系和 JSONL 日志
- **安全导入**：导入前检查清单与日志归属，拒绝覆盖本机同 ID 会话
- **保留原工作目录**：导入后可根据新电脑的实际目录继续执行路径修复

### 🗑️ 删除与备份管理

- **普通会话删除**：调用 Codex app-server 删除主会话及其子代理
- **遗留日志删除**：仅允许删除受管理目录中未被数据库引用的 JSONL
- **删除回退**：可从删除历史恢复会话、项目归属和日志
- **集中备份管理**：查看、打开和清理修复、删除、导入及回退产生的备份

### 🖥️ 桌面体验

- macOS 与 Windows 路径及进程管理兼容
- 深色与浅色主题
- 中文原生菜单和常用缩放快捷键
- 单实例运行，重复启动时聚焦现有窗口
...
<div align="center">
  <img src="./screenshot/软件主页.png" alt="Codex 会话管理的会话审计与路径核对主界面" width="100%" />
</div>
...
可通过顶部“备份目录”选择其他绝对路径，并在“备份历史”页面集中查看或清理备份。

## 🛡️ 安全机制

- 所有修复、删除、导入和回退操作均先创建本地备份
...
- 删除目标限制在 Codex 管理的数据范围内，并保留可审计操作清单
...
| 操作备份 | `~/.codex/session-manager-backups/` | 修复、删除、导入与回退备份 |
